...
\end{align}
where we assume access to first-order queries for $f$. For our eventual application to diffusion models, we will choose $f=\log p_t$ in view of \cref{eq:backward_kernel} and approximate $\nabla f\approx \scf_t$. 
...
To perform rejection sampling, we have to appropriately choose the proposal distribution $q$. By Taylor's expansion, for a fixed  $\xp\in\RR^d$, we have
\begin{align*}
...
\end{aligned}
\end{align}
so that $\gamp{\lr}=\alrp (x -\xz) + \blrp z$.
...
\cref{thm:gaussian_tilt} interpolates between the Lipschitz case $s=0$, which requires $\eta^{-1} \gg \beta_1^2 \log(1/\delta)$, and the smooth case $s=1$, which requires $\eta^{-1} \gg \beta_2\, d^{1/2} \log(1/\delta)$.
